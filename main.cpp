...
int main() {
    ...
    ///// FILL HERE /////
    bool rebellion = 0;  //added to be used in removeLand() function
    int myManpower = 3; //added by me as default data of player character
    int myGold = 20;
    int myNumOfLands = 1;
    string myHolding = "village";

    Land myLand(land, myHolding);
    Character myCharacter(name, myManpower, myGold, myNumOfLands);
    myCharacter.addLand(myLand);

    const Character general(general);
    ...
    while (choice != 6) {
        ...
        case 1: {
            ...
            cout << "You've talked with your general " << /* get the name of general */general.nameGetter() << "." << endl; 
        ...
        case 2: {
            ...
            */
            Land landFinder(string &landname)
            {
                for (int i = 0; i < charList.sizeGetter(); i++)
                {
                    if (charactersGetter()[i]->nameGetter().compare(getLandOwner(landname)) == 0)
                    {
                        Land* iter = getLandOwner(land).landsGetter();
                        for (int j = 0; j < getLandOwner(landname).numOfLandsGetter(); j++)
                        {
                            if (charactersGetter()[i]->nameGetter().compare(landname) == 0)
                            {
                                return *iter;
                            }
                            else
            ...
            for (int i = 0; i < charList.sizeGetter(); i++)
            {
                if (charactersGetter()[i]->nameGetter().compare(getLandOwner(land)) == 0)
                {
                    if (charactersGetter()[i]->manpowerGetter() < myCharacter.manpowerGetter())
                    {
                        int n = myCharacter.manpowerGetter() - charactersGetter()[i]->manpowerGetter();

                        myCharacter.manpowerSetter(n);

                        charactersGetter()[i]->manpowerGetter() = 0;
                        
                        myCharacter.addLand(landFinder(land));
                    ...
                    else
                    {
                        ...
                        cout << "You've lost the battle and " << myCharacter.manpowerGetter() << " manpower." << endl;

                        myCharacter.manpowerSetter(num);
        ...
        case 3:
            ...
            */
            ...
            if (myCharacter.goldGetter() > order * 5)
            {
                int number = myCharacter.manpowerGetter();
                myCharacter.manpowerSetter(number + order);
                cout << "Order successful. You have " << myCharacter.manpowerGetter() << " manpower." << endl;
            }
            else
            {
...
void readData(string fileName, CharacterList& charList) {
    ...
    if (dataFile.is_open()) {
        ...
        while (dataFile >> word) {
            ...
            Character newChar (name, manpower, money, numOfLands);
...
void CharacterList::listCharacters(CharacterList& charList) {
    cout << endl;
    ...
    {
        cout << i + 1 << ". " << characters[i].nameGetter() << endl;
    }
...
void CharacterList::listLands(CharacterList& charList) {
    cout << endl;
    ...
    */
    ...
    for (int i = 0; i < size; i++)
    {
        ...
        {
            cout << iter->nameGetter() << " " << iter->holdingGetter() << " owned by " << getLandOwner(iter->nameGetter()) << endl;
            iter = iter->nextGetter();
        }
    }
...
bool endOfTurn(Character& player, int round) { //return a bool value that indicates if the game is over or not.
    ...
    */
    ...
    myCharacter.getTaxes();
    if (myCharacter.numOfLandsGetter() == 0) {
        cout << "You are no longer a great warrior. You survived " << round << " turns." << endl;
        cout << endl;
        cout << "GAME OVER." << endl;
    }
    if (myCharacter.manpowerGetter() == 0) {
        rebellion = 1;
